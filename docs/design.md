...
Both firmwares are supported from the same files, and every difference is capability-detected at the point of use (check whether the running firmware has the surface, fall back otherwise), never firmware-identified. Exactly three divergences exist, each verified against both source trees:
...
- `printer.wait_while` postdates stock Klipper. The temperature wait uses it when present; otherwise it runs the manual loop stock's own `TEMPERATURE_WAIT` uses (poll while the printer is not shut down, `reactor.pause` one second ahead). Both branches share the same pure wait decision, including the cancel-on-cleared-target rule.
