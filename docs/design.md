...
A single physical filament stepper serves every tool: it lives in the one extruder section configured with step pins (T0's), derived at connect rather than configured, and it is a connect-time error if zero or more than one tool's extruder section carries a stepper. On every activation the plugin issues `SYNC_EXTRUDER_MOTION` to attach that stepper to the active extruder section's motion queue (heater-only extruder sections still allocate a motion queue, so the attachment works); when detection resolves to no tool mounted, or a mid-print tool loss fires, the stepper is detached with an empty `MOTION_QUEUE`, so `G1 E` moves no motor until the next activation.
...
- `HOTENDCHANGER_STATUS`: prints labeled rows: active tool, detected tool, current state, each tool's detect pin reading (when pins are configured), the toolhead detect pin reading (when configured), the extruder stepper's current motion queue (`none` when detached; also `none` at boot before any activation, when the stepper still drives its own section by construction), and per tool one row with the current X/Y/Z offset and the extruder section name.
...
A mid-print tool loss (see the detection section) leaves the same footprint as a failed change: state `error`, no active tool, extruder stepper detached; `INITIALIZE_HOTENDCHANGER` (or a manual re-seat plus the automatic rediscovery, when no print is running) resolves it.
